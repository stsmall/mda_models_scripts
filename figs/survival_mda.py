...
##################################
    ...
    if clear_count == 1:
        ...
        mfdiemda = dfworm.meta.ix[MFmda].groupby("hostidx").apply(lambda y: np.where(np.random.random(y.shape[0]) < mda_micro ** y.fitS))
        juvdiemda = dfworm.meta.ix[Juvmda].groupby("hostidx").apply(lambda y: np.where(np.random.random(y.shape[0]) < mda_juvicide ** y.fitS))
        adultdiemda = dfworm.meta.ix[Adultmda].groupby("hostidx").apply(lambda y: np.where(np.random.random(y.shape[0]) < mda_macro ** y.fitS))        
        diemda = [i for i in mfdiemda.values] + [i for i in juvdiemda.values] + [i for i in adultdiemda.values]
        dfworm.drop_worms(np.concatenate(diemda).ravel())
    else: pass   
#########################################
