...
def migration_matrix(villages,initial_migration,initial_distance_m,theta,basepairs,mutation):
    ...
    if villages > 4: #cant figure out how to pythonically increase the villages without just using loops
        ...
        if len(distance_m) != ((villages)*(villages-1)/2): #check that number of distances is appropriate for number of villages 
...
def trans_init(prev,hostpopsize,villages,muTrans,sigma,sizeTrans):
...
def worm_burden(prev,hostpopsize,muWormBurden,sizeWormBurden,villages,initial_migration,initial_distance_m,theta,basepairs,mutation,recombination,time2Ancestral,thetaAncestral,thetaRegional,time_join12,time_join23,time_join34,muTrans,sizeTrans,sigma):
    ...
    transmission_mat,dispersal=trans_init(prev,hostpopsize,villages,muTrans,sigma,sizeTrans)
...
def maturation(meta_popdict,month,prev_t,density_dependence,mortalityHost,mortalityAdult,mortalityJuv,mortalityMF,fecundity,basepairs,mutation,recombination):
    ...
    #month: (int) what month is it 1 ... 12
    #prev_t: (float) current prevalance at time
    ...
    if month%12 is 0: #this denotes 1 year has passed so adults mature to next age class 
        for mpop in meta_popdict.keys(): #villages 
            for npop in meta_popdict[mpop].keys(): #infected hosts as pops within villages
                ...
                if sum(num_recomb) != 0:
                    mf = recombination(mf,num_recomb)
...
def recombination(mf,hap_pop,num_recomb,basepairs):
       ...
       for i in range(1,basepairs): #number of muts
          ...
          while recomb < num_recomb[i-1]: #keep going until all recombinations are  assigned
               ...
               if new_recomb < 2: #first parent
                      ...
                      crossover_pos = basepairs[i]/2 + basepairs[i]*np.random.normal(0,0.1,1)
                      hap1.sort()
                      hap2.sort()
                      ...
                      hap3 = hap1[0:hap1_co] + hap2[hap2_co:]
                      hap4 = hap2[0:hap2_co] + hap1[hap1_co:]
                      mf[rec_mf][i+1][0] = hap3
                      mf[rec_mf][i+1][1] = hap4
               else:#second parent
                      hap1 = mf[rec_mf][i+1][2]
                      hap2 = mf[rec_mf][i+1][3]
                      crossover_pos = basepairs[i]/2 + basepairs[i]*np.random.normal(0,0.1,1)
                      hap1.sort()
                      hap2.sort()
                      hap1_co = next(l[0] for l in enumerate(hap1) if l[1] > crossover_pos)
                      ...
                      hap3 = hap1[0:hap1_co] + hap2[hap2_co:]
                      hap4 = hap2[0:hap2_co] + hap1[hap1_co:]
                      mf[rec_mf][i+1][2] = hap3
                      mf[rec_mf][i+1][3] = hap4 
               recomb += 1                                
       return mf
